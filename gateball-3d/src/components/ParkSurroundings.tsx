...
export function Clubhouse() {
  ...
  const xCenter = 24.25; // midpoint of front wall (x=21) and back wall (x=27.5)
  const zCenter = 0.0;
  ...
  // Gold on bronze "GATEBALL CLUB" plaque canvas texture
  const signTexture = useMemo(() => {
    ...
    const ctx = canvas.getContext('2d');
    if (ctx) {
      // Dark bronze metallic gradient background
      const bgGrad = ctx.createLinearGradient(0, 0, 1024, 256);
      bgGrad.addColorStop(0, '#1c1309');
      bgGrad.addColorStop(0.5, '#422c15');
      bgGrad.addColorStop(1, '#1c1309');
      ctx.fillStyle = bgGrad;
      ctx.fillRect(0, 0, 1024, 256);

      // Gold bronze double border
      ...
      ctx.fillText('GATEBALL CLUB', 512, 128);
    }
    const texture = new CanvasTexture(canvas);
    return texture;
  }, []);
  ...
  return (
    <group>
      ...
      {/* 8. Gold and Bronze "GATEBALL CLUB" Logo Plaque Sign */}
      <group position={[17.34, 2.45, zCenter]}>
        <mesh position={[0, 0, 0]} castShadow>
...
export default function ParkSurroundings() {
  // Bounding box defined by the official court dimensions of 35 standard yards by 28 standard yards scaled down 20%
  const courtLength = 28.0; // yards (North-South)
  const courtWidth = 22.4;  // yards (East-West)
  const outerMargin = 3.2; // Consistent, uniform outer margin for spectator seating

  // Corner coordinates for the enclosing rectangular loop
  const boundaryX = courtWidth / 2 + outerMargin;   // 14.4 yards
  const boundaryZ = courtLength / 2 + outerMargin;  // 17.2 yards
  ...
  return (
    <group>
      ...
      {/* Sideline Seats (West) */}
      <ShadedBench position={[-12.4, 0, -6]} rotation={[0, Math.PI / 2, 0]} />
      <ShadedBench position={[-12.4, 0, 6]} rotation={[0, Math.PI / 2, 0]} />
      ...
      {/* Endline Seats (North) */}
      <ShadedBench position={[-4, 0, -15.2]} rotation={[0, 0, 0]} />
      <ShadedBench position={[4, 0, -15.2]} rotation={[0, 0, 0]} />

      {/* Endline Seats (South) */}
      <ShadedBench position={[-4, 0, 15.2]} rotation={[0, Math.PI, 0]} />
      <ShadedBench position={[4, 0, 15.2]} rotation={[0, Math.PI, 0]} />

      {/* 8-Foot Hedge running outside the West (Blue/Red corner) fence */}
      <Hedge start={[-16.4, -18.2]} end={[-16.4, 18.2]} />
