...
function HalfwayPeg({ position, placementStep, currentStep }: HalfwayPegProps) {
  ...
  // Proportions scaled to match the visual scale of the balls (ball radius 0.15 vs real 0.0475).
  // Real spec: height 20cm, diameter 2cm (ratio 10:1). Scaled: height 0.60, radius 0.030.
  const pegHeight = 0.60;
  const pegRadius = 0.030;

  return (
    <group ref={groupRef} position={position}>
      {/* Shaft — brushed steel: medium roughness, very high metalness */}
      <mesh position={[0, pegHeight / 2, 0]} castShadow receiveShadow>
        <cylinderGeometry args={[pegRadius, pegRadius * 1.04, pegHeight, 32]} />
      ...
      {/* Rounded top cap — slightly more polished than shaft */}
      <mesh position={[0, pegHeight, 0]}>
        <sphereGeometry args={[pegRadius, 24, 24]} />
        <meshStandardMaterial
          color="#9aa2aa"
          metalness={0.92}
          roughness={0.18}
...
export default function CourtMeasurements() {
  ...
  // Initial load hook: start in Top-down landscape view
  useEffect(() => {
        ...
        {step >= 11 && (
          <group>
            ...
            <Arrow3D 
              ...
              label={(
                <div style={{ textAlign: 'center' }}>
                  <div>Penalty Area: {unit === 'yards' ? '1 yd' : unit === 'metres' ? '0.91 m' : '1 yd / 0.91 m'}</div>
