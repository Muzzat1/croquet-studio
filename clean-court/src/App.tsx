...
export default function App() {
  ...
  const handleCourtPointerDown = (e: any) => {
    ...
    if (drawMode) {
      ...
      const pt = e.point;
      if (pt) {
        drawStartPoint.current = [pt.x, 0.115, pt.z];
        setCurrentDrawingPoints([[pt.x, 0.115, pt.z]]);
      }
      return;
    }
      ...
      if (pt && drawStartPoint.current) {
        if (drawTool === 'freehand') {
          setCurrentDrawingPoints(prev => {
            if (prev.length === 0) return [[pt.x, 0.115, pt.z]];
            const lastPoint = prev[prev.length - 1];
            const dx = pt.x - lastPoint[0];
            const dz = pt.z - lastPoint[2];
            const dist = Math.sqrt(dx * dx + dz * dz);
            if (dist > 0.01) {
              return [...prev, [pt.x, 0.115, pt.z]];
            }
            return prev;
          });
      ...
      <Canvas camera={{ position: [-31.49, 13.87, -0.11], fov: 45.0, far: 5000 }} shadows>
        ...
        {(() => {
          ...
          return (
            <Line 
              points={[
                [balls[selectedBall].x, 0.115, balls[selectedBall].z],
                [hoverPoint.x, 0.115, hoverPoint.z]
              ]} 
              color={
