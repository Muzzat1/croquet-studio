...
import { to3DX, to3DZ, to2DX, to2DY, DISPLAY_RADIUS, SCALE } from '../../physics/CroquetPhysics';
...
const CroquetBall = forwardRef<THREE.Mesh, CroquetBallProps>(
  ({ color, x, y, onPositionChange, ...props }, ref) => {
    // Visual radius matching 2D proportions basis (6 pixels / 22 scale = 0.27273 yards)
    const radius = DISPLAY_RADIUS / SCALE;

    const [isDragging, setIsDragging] = useState(false);

    // Retrieve R3F state properties and store them in mutable refs.
    ...
    const handlePointerMove = (e: any) => {
      ...
      if (raycasterRef.current && raycasterRef.current.ray) {
        ...
        // Convert 3D coordinates (x3d, z3d) back to 2D coordinates (x2d, y2d)
        const x2d = to2DX(intersectionPoint.current.z);
        const y2d = to2DY(intersectionPoint.current.x);
        
        onPositionChange(x2d, y2d);
      }
    };
