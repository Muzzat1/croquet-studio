...
import React, { useState, useRef, useEffect, useCallback } from 'react';import { Play, RotateCcw, Search, MousePointer2, Maximize2, Minimize2, HelpCircle, X, Eye, Clapperboard, ChevronLeft, ChevronRight, Save, FolderUp, Trash2, MonitorPlay, Camera, ZoomIn, ZoomOut, Pencil, Eraser, Settings, Undo2, Minus, Plus } from 'lucide-react';
...
const HOOPS = [
  { id: 1, x: EDGING + 7 * SCALE, y: EDGING + 7 * SCALE, label: '1', topColor: '#2563eb', acLabel: '2 Back' },
  { id: 2, x: EDGING + 28 * SCALE, y: EDGING + 7 * SCALE, label: '2', acLabel: '1 Back' },
  { id: 3, x: EDGING + 28 * SCALE, y: EDGING + 21 * SCALE, label: '3', acLabel: '4 Back' },
  { id: 4, x: EDGING + 7 * SCALE, y: EDGING + 21 * SCALE, label: '4', acLabel: '3 Back' },
  { id: 5, x: EDGING + 10.5 * SCALE, y: EDGING + 14 * SCALE, label: '5', topColor: '#dc2626', acLabel: 'Rover' },
  { id: 6, x: EDGING + 24.5 * SCALE, y: EDGING + 14 * SCALE, label: '6', acLabel: 'Penultimate' },
];
...
const CROQUET_SHOT_TYPES = [
  { id: 'stop', label: 'Stop Shot', CT: 1.2, CS: 0.35, CLat: 0.9 },
  { id: 'drive', label: 'Drive', CT: 1.1, CS: 0.49, CLat: 0.9 },
  { id: 'half-roll', label: 'Half Roll', CT: 1.0, CS: 0.71, CLat: 0.9 },
  { id: 'roll', label: 'Full Roll', CT: 0.9, CS: 0.9, CLat: 0.9 },
  { id: 'pass-roll', label: 'Pass Roll', CT: 0.8, CS: 1.15, CLat: 0.9 }
];
...
export default function App() {
  ...
  const [stillBallMessage, setStillBallMessage] = useState<{ x: number; y: number } | null>(null);
  ...
  const [croquetShotType, setCroquetShotType] = useState('drive');
  const [croquetRotationAngle, setCroquetRotationAngle] = useState(0);
const touchingBallId = React.useMemo(() => {
  ...
  const others = [blue, red, yellow, black].filter(b => b.id !== activeBallId && !isBallDocked(b));
  ...
  for (const b of others) {
    ...
    if (dist <= 2 * DISPLAY_RADIUS + 0.5) {
      // --- ADD THIS LINE TO SYNC THE DIAL ---
      const initialAngle = (Math.atan2(dy, dx) * 180) / Math.PI;
      setCroquetRotationAngle(initialAngle); 
      return b.id;
    }
  }
  return null;
}, [activeBallId, blue, red, yellow, black, isPlaying, isReplaying]);
// Link the 3D dial rotation to physical ball movement on the court
useEffect(() => {
  // Only run this if we have a touching ball and we are NOT currently in the middle of a shot
  if (touchingBallId && !isPlaying && !isReplaying) {
    const angleRad = (croquetRotationAngle * Math.PI) / 180;
    const otherBalls = [blue, red, yellow, black];
    const targetBall = otherBalls.find(b => b.id === touchingBallId);
    
    if (targetBall) {
      const distance = DISPLAY_RADIUS * 2 + 0.1; 
      const newX = targetBall.x + Math.cos(angleRad) * distance;
      const newY = targetBall.y + Math.sin(angleRad) * distance;
      
      const updateBall = (prev: Ball) => ({ ...prev, x: newX, y: newY });
      
      // Use functional updates to ensure we don't interfere with other state changes
      if (activeBallId === 'blue') setBlue(updateBall);
      else if (activeBallId === 'red') setRed(updateBall);
      else if (activeBallId === 'yellow') setYellow(updateBall);
      else if (activeBallId === 'black') setBlack(updateBall);
    }
  }
  // Remove 'angle' or 'speed' from this specific dependency array 
  // to ensure moving the inner wheel doesn't re-trigger a physical position snap.
}, [croquetRotationAngle, touchingBallId, activeBallId]);
  const ignoreCollisionsRef = useRef<string[]>([]);
    ...
    setSequence(prev => {
      ...
      if (newSeq.length > currentShotIndex) {
        newSeq[currentShotIndex] = {
          ...newSeq[currentShotIndex],
          positions: { blue: { ...blueRef.current, vx: 0, vy: 0 }, red: { ...redRef.current, vx: 0, vy: 0 }, yellow: { ...yellowRef.current, vx: 0, vy: 0 }, black: { ...blackRef.current, vx: 0, vy: 0 } },
  ...
  const playShot = () => {
    ...
    let isCroquetShot = false; let croquetTarget: Ball | null = null;
    for (let i = 0; i < bs.length; i++) {
       for (let j = i + 1; j < bs.length; j++) {
          const dx = bs[i].x - bs[j].x; const dy = bs[i].y - bs[j].y;
          if (Math.sqrt(dx*dx + dy*dy) <= 2 * DISPLAY_RADIUS + 0.5 && !isBallDocked(bs[i]) && !isBallDocked(bs[j])) {
             pairs.push(`${bs[i].id}-${bs[j].id}`);
             if (bs[i].id === activeBallId) { croquetTarget = bs[j]; isCroquetShot = true; }
             if (bs[j].id === activeBallId) { croquetTarget = bs[i]; isCroquetShot = true; }
          }
       }
    }
    ...
    const M = { x: Math.sin(rad), y: -Math.cos(rad) };
    if (isCroquetShot && croquetTarget) {
      const shotConfig = CROQUET_SHOT_TYPES.find(s => s.id === croquetShotType) || CROQUET_SHOT_TYPES[1];
      const nx = croquetTarget.x - activeBall.x; const ny = croquetTarget.y - activeBall.y;
      const dist = Math.sqrt(nx*nx + ny*ny);
      const N = { x: nx/dist, y: ny/dist };
      const cos_theta = M.x * N.x + M.y * N.y;
      if (cos_theta > 0) {
         playSound(SOUNDS.mallet, 0.6);
         ignoreCollisionsRef.current = [`${activeBall.id}-${croquetTarget.id}`, `${croquetTarget.id}-${activeBall.id}`];
         const v_T_mag = initialSpeed * cos_theta * shotConfig.CT;
         const v_S_forward_mag = initialSpeed * cos_theta * shotConfig.CS;
         const M_lat = { x: M.x - N.x * cos_theta, y: M.y - N.y * cos_theta };
         const v_S_lat_x = M_lat.x * initialSpeed * shotConfig.CLat;
         const v_S_lat_y = M_lat.y * initialSpeed * shotConfig.CLat;
         const t_vx = N.x * v_T_mag; const t_vy = N.y * v_T_mag;
         const s_vx = N.x * v_S_forward_mag + v_S_lat_x; const s_vy = N.y * v_S_forward_mag + v_S_lat_y;
         if (activeBallId === 'blue') blueRef.current = { ...blueRef.current, vx: s_vx, vy: s_vy };
         else if (activeBallId === 'red') redRef.current = { ...redRef.current, vx: s_vx, vy: s_vy };
         else if (activeBallId === 'yellow') yellowRef.current = { ...yellowRef.current, vx: s_vx, vy: s_vy };
         else if (activeBallId === 'black') blackRef.current = { ...blackRef.current, vx: s_vx, vy: s_vy };
         if (croquetTarget.id === 'blue') blueRef.current = { ...blueRef.current, vx: t_vx, vy: t_vy };
         else if (croquetTarget.id === 'red') redRef.current = { ...redRef.current, vx: t_vx, vy: t_vy };
         else if (croquetTarget.id === 'yellow') yellowRef.current = { ...yellowRef.current, vx: t_vx, vy: t_vy };
         else if (croquetTarget.id === 'black') blackRef.current = { ...blackRef.current, vx: t_vx, vy: t_vy };
         setTargetSpot(null);
         setIsPlaying(true);
         return;
      } else {
         setStillBallMessage({ x: croquetTarget.x, y: croquetTarget.y });
         setTimeout(() => setStillBallMessage(null), 3000);
      }
    }
    playSound(SOUNDS.mallet, 0.6);
    const vx = M.x * initialSpeed; const vy = M.y * initialSpeed;
  ...
  const drawField = (ctx: CanvasRenderingContext2D) => {
    ...
    HOOPS.forEach(hoop => {
      ...
      if (!cleanFeed) { 
        ...
        if ((hoop as any).acLabel) {
          ctx.fillStyle = 'rgba(255, 255, 255, 0.9)'; 
          ctx.font = '11px "Inter", sans-serif'; 
          ctx.fillText((hoop as any).acLabel, hoop.x, hoop.y - yOffset + 4);
        }
      }
    });
    const flags = [
    ...
    if (!cleanFeed) {
      ...
      ctx.strokeRect(EDGING + SCALE, EDGING + SCALE, 33 * SCALE, 26 * SCALE);
      ctx.strokeStyle = 'rgba(255, 255, 255, 0.8)'; ctx.lineWidth = 3;
      ctx.beginPath();
      ctx.moveTo(EDGING + SCALE, EDGING + SCALE);
      ctx.lineTo(EDGING + SCALE, EDGING + 14 * SCALE);
      ctx.stroke();
      ctx.beginPath();
      ctx.moveTo(EDGING + 34 * SCALE, EDGING + 27 * SCALE);
      ctx.lineTo(EDGING + 34 * SCALE, EDGING + 14 * SCALE);
      ctx.stroke();
      const sidelinePegs = [
        { x: EDGING + SCALE, y: EDGING }, { x: EDGING, y: EDGING + SCALE },
        { x: EDGING + 34 * SCALE, y: EDGING }, { x: EDGING + 35 * SCALE, y: EDGING + SCALE },
        { x: EDGING + 34 * SCALE, y: EDGING + 28 * SCALE }, { x: EDGING + 35 * SCALE, y: EDGING + 27 * SCALE },
        { x: EDGING + SCALE, y: EDGING + 28 * SCALE }, { x: EDGING, y: EDGING + 27 * SCALE }
      ];
      sidelinePegs.forEach(peg => {
  ...
  const drawPrediction = (ctx: CanvasRenderingContext2D) => {
    ...
    if (touchingBallId) {
       ...
       const targetBall = otherBalls.find(b => b.id === touchingBallId);
       if (targetBall) {
          const nx = targetBall.x - activeBall.x; const ny = targetBall.y - activeBall.y;
          const dist = Math.sqrt(nx*nx + ny*ny);
          const N = { x: nx/dist, y: ny/dist };
          const cos_theta = dx * N.x + dy * N.y;
          if (cos_theta > 0) {
             const shotConfig = CROQUET_SHOT_TYPES.find(s => s.id === croquetShotType) || CROQUET_SHOT_TYPES[1];
             const arrowLen = 35;
             const v_S_forward_mag = cos_theta * shotConfig.CS;
             const M_lat = { x: dx - N.x * cos_theta, y: dy - N.y * cos_theta };
             const v_S_lat_x = M_lat.x * shotConfig.CLat;
             const v_S_lat_y = M_lat.y * shotConfig.CLat;
             const s_dx = N.x * v_S_forward_mag + v_S_lat_x;
             const s_dy = N.y * v_S_forward_mag + v_S_lat_y;
             drawArrow(ctx, targetBall.x, targetBall.y, targetBall.x + N.x * arrowLen, targetBall.y + N.y * arrowLen, targetBall.color + 'CC');
             const sMag = Math.sqrt(s_dx*s_dx + s_dy*s_dy);
             if (sMag > 0.05) {
                drawArrow(ctx, activeBall.x, activeBall.y, activeBall.x + (s_dx/sMag) * arrowLen, activeBall.y + (s_dy/sMag) * arrowLen, activeBall.color + 'CC');
             }
             return;
          }
       }
        ...
        for (let i = 0; i < balls.length; i++) {
          for (let j = i + 1; j < balls.length; j++) {
            ...
            if (distSq < minContactDistSq) {
              ...
              if (ignoreCollisionsRef.current.includes(pairStr1) || ignoreCollisionsRef.current.includes(pairStr2)) {
                 continue;
              }
    ...
    <>
      ...
      <div className="h-screen w-screen bg-zinc-900 flex flex-row overflow-hidden relative selection:bg-emerald-500/30 text-zinc-100 select-none" style={{ WebkitTouchCallout: 'none', WebkitUserSelect: 'none', userSelect: 'none' }}>
        ...
        {!cleanFeed && (
          <aside className="relative z-10 w-[240px] md:w-[320px] lg:w-[360px] h-full flex flex-col shrink-0 backdrop-blur-xl border-r p-3 md:p-5 overflow-y-auto custom-scrollbar shadow-[8px_0_30px_rgba(0,0,0,0.5)] transition-colors duration-300 bg-zinc-900/90 border-zinc-700/50">
            ...
            <div className="shrink-0 grid grid-cols-4 gap-1.5 md:gap-2 mb-3 md:mb-4 pb-3 md:pb-4 border-b border-zinc-800/50">
              <ToolButton icon={<MousePointer2 size={14} className="md:w-[18px] md:h-[18px]" />} active={placementMode && !drawMode} label="PLACE" title="Click on a ball to highlight and click the crosshair on red spot" onClick={() => { setPlacementMode(true); setGhostBallEnabled(false); setTargetSpot(null); setDrawMode(false); setDrawWarning(false); }} />
              <ToolButton icon={<Eye size={14} className="md:w-[18px] md:h-[18px]" />} active={!placementMode && !drawMode} label="AIM" title="Use the Velocity and Aim wheels to direct your shot" onClick={() => { setPlacementMode(false); setGhostBallEnabled(true); setTargetSpot(null); setDrawMode(false); setDrawWarning(false); }} />
              <ToolButton icon={<Pencil size={14} className="md:w-[18px] md:h-[18px]" />} active={drawMode} label="DRAW" title="Draw tactical lines on the court" onClick={() => { const newMode = !drawMode; setDrawMode(newMode); if (newMode) { setPlacementMode(false); setTargetSpot(null); } }} />
              <ToolButton icon={isFullscreen ? <Minimize2 size={14} className="md:w-[18px] md:h-[18px]" /> : <Maximize2 size={14} className="md:w-[18px] md:h-[18px]" />} label={isFullscreen ? "REDUCE" : "EXPAND"} title="Toggle Fullscreen" onClick={toggleFullscreen} />
            </div>
            <div className={`shrink-0 relative flex flex-col gap-3 md:gap-4 mb-3 md:mb-4 pb-3 md:pb-4 border-b border-zinc-800/50 transition-opacity min-h-[80px] ${!activeBallId ? 'opacity-30 pointer-events-none' : ''}`}>
              {!placementMode ? (
                <SphericalController
                  ...
                  setSpeed={setSpeed}
                  isPlaying={isPlaying}
                  isCroquetShot={!!touchingBallId} 
                  contactAngle={croquetRotationAngle}
                  setContactAngle={setCroquetRotationAngle}
                />
              ) : (
            ...
            <div className="shrink-0 flex justify-center gap-2 md:gap-4 mb-4 pb-4 border-b border-zinc-800/50">
              ...
              <button onClick={() => setShowOptions(true)} className="flex items-center gap-2 px-3 md:px-4 py-1.5 md:py-2 rounded-full transition-all border shadow-sm bg-zinc-900/60 hover:bg-zinc-800 text-zinc-400 hover:text-emerald-400 border-zinc-800" title="Studio Preferences">
                <Settings size={14} /><span className="text-[9px] md:text-[10px] font-bold uppercase tracking-widest">Prefs</span>
              </button>
              <button onClick={() => setShowHelp(true)} className="flex items-center gap-2 px-3 md:px-4 py-1.5 md:py-2 rounded-full transition-all border shadow-sm bg-zinc-900/60 hover:bg-zinc-800 text-zinc-400 hover:text-emerald-400 border-zinc-800" title="Open Help Guide">
                <HelpCircle size={14} /><span className="text-[9px] md:text-[10px] font-bold uppercase tracking-widest">HELP</span>
              </button>
            </div>
            {features.zoom && (
        ...
        <main className="relative z-10 flex-1 h-full overflow-hidden bg-black/20" ref={viewportRef}>
          <AnimatePresence>
            {drawMode && !cleanFeed && (
              <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className={`fixed top-2 md:top-4 left-1/2 -translate-x-1/2 z-[100] pointer-events-auto flex items-center gap-1 md:gap-2 p-1.5 rounded-full shadow-[0_8px_30px_rgba(0,0,0,0.3)] border-2 backdrop-blur-xl transition-all duration-300 ${drawWarning ? 'bg-amber-950/95 border-amber-500 scale-105' : 'bg-zinc-900/95 border-emerald-500'}`}>
                <div className={`flex items-center gap-2 pl-3 pr-2 py-1 ${drawWarning ? 'text-amber-400' : 'text-emerald-400'}`}>
                  ...
                  <span className="text-[10px] md:text-xs font-bold uppercase tracking-widest whitespace-nowrap">{drawWarning ? "Turn off Draw to select" : "Telestrator Active"}</span>
                </div>
                <div className="w-px h-5 bg-zinc-700/50"></div>
                <div className="flex bg-zinc-950/50 rounded-full p-0.5">
                  <button onClick={(e) => { e.stopPropagation(); setDrawStyle('freehand'); }} className={`p-1.5 rounded-full transition-colors ${drawStyle === 'freehand' ? 'bg-zinc-700 text-white' : 'text-zinc-500 hover:text-zinc-300'}`}><Pencil size={15} /></button>
                  <button onClick={(e) => { e.stopPropagation(); setDrawStyle('straight'); }} className={`p-1.5 rounded-full transition-colors ${drawStyle === 'straight' ? 'bg-zinc-700 text-white' : 'text-zinc-500 hover:text-zinc-300'}`}><Minus size={15} /></button>
                </div>
                ...
                <div className="w-px h-5 bg-zinc-700/50"></div>
                <button onClick={(e) => { e.stopPropagation(); setDrawings(prev => prev.slice(0, -1)); }} disabled={drawings.length === 0} className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[10px] md:text-xs font-bold uppercase tracking-widest text-zinc-300 hover:text-white hover:bg-zinc-800 disabled:opacity-30 transition-colors"><Undo2 size={14} /> <span className="hidden md:inline">Undo</span></button>
                <button onClick={(e) => { e.stopPropagation(); setDrawings([]); setDrawMode(false); setDrawWarning(false); }} className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[10px] md:text-xs font-bold uppercase tracking-widest text-rose-400 hover:text-rose-300 hover:bg-rose-500/10 transition-colors"><Eraser size={14} /> <span className="hidden md:inline">Clear</span></button>
                <div className="w-px h-5 bg-zinc-700/50"></div>
                <button onClick={(e) => { e.stopPropagation(); setDrawMode(false); setDrawWarning(false); }} className="p-1.5 mr-1 rounded-full text-zinc-400 hover:text-white hover:bg-zinc-800 transition-colors"><X size={16} /></button>
              </motion.div>
            )}
          </AnimatePresence>
          <div className={zoom === 1 ? "absolute inset-0 flex items-center justify-center min-w-0 min-h-0" : "min-w-max min-h-max p-12 flex items-center justify-center overflow-auto"}>
            <div style={getCanvasStyle()} className={`shadow-[0_0_40px_rgba(0,0,0,0.5)] bg-transparent pointer-events-auto border ${cleanFeed ? 'border-transparent' : 'border-zinc-800/80'} rounded-lg ${draggingItem === 'pan' ? 'cursor-grabbing' : (!isPlaying && !cleanFeed ? (drawMode ? 'cursor-crosshair' : (placementMode ? 'cursor-crosshair' : 'cursor-grab')) : 'cursor-default')} touch-none`}>
              <div className={zoom === 1 ? "absolute inset-0" : "w-full h-full relative"}>
                <canvas ref={canvasRef} style={{ display: 'block', width: '100%', height: '100%' }} className="rounded-lg" onMouseDown={handleStart} onMouseMove={handleMove} onMouseUp={handleMouseUp} onMouseLeave={handleMouseLeave} onTouchStart={handleStart} onTouchMove={handleMove} onTouchEnd={handleMouseUp} />
                
               {/* Shot Type Selector (Drive, Roll, etc) */}
{touchingBallId && !placementMode && !cleanFeed && (
  <div className="absolute bottom-4 left-1/2 -translate-x-1/2 z-30 flex flex-col items-center gap-3">
    <div className="flex bg-zinc-950/90 border border-emerald-500/80 shadow-lg rounded-full p-1 w-max pointer-events-auto backdrop-blur-xl">
      {CROQUET_SHOT_TYPES.map(s => (
        <button key={s.id}
          onClick={(e) => { e.stopPropagation(); setCroquetShotType(s.id); }}
          className={`flex-1 whitespace-nowrap px-2 py-1.5 text-[9px] md:text-[10px] font-black tracking-widest uppercase rounded-full transition-all ${croquetShotType === s.id ? 'bg-emerald-500 text-zinc-950 shadow-md' : 'text-zinc-400 hover:text-emerald-300 hover:bg-zinc-800/80'}`}
        >
          {s.label}
        </button>
      ))}
                ...
                <AnimatePresence>
                  {stillBallMessage && (
                    <motion.div
                      ...
                      style={{
                        left: `${(stillBallMessage.x / FIELD_WIDTH) * 100}%`,
                        top: `${((stillBallMessage.y - DISPLAY_RADIUS - 10) / FIELD_HEIGHT) * 100}%`,
                        transform: 'translate(-50%, -100%)'
                      }}
                    >
                      <div className="bg-rose-500/90 text-white text-[10px] md:text-xs font-bold px-2.5 py-1 rounded-md shadow-lg whitespace-nowrap uppercase tracking-wider backdrop-blur-sm border border-rose-400">
                        Still Ball
                      </div>
                    </motion.div>
                  )}
