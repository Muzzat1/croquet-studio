...
import React, { useState, useRef, useEffect, useCallback } from 'react';import { Play, RotateCcw, Search, MousePointer2, Maximize2, Minimize2, HelpCircle, X, Eye, Clapperboard, ChevronLeft, ChevronRight, Save, FolderUp, Trash2, MonitorPlay, Camera, ZoomIn, ZoomOut, Pencil, Eraser, Settings, Undo2, Hand, Sun, Minus } from 'lucide-react';
...
useEffect(() => {
  ...
  if (touchingBallId && !isPlaying && !isReplaying) {
    ...
    if (targetBall) {
      const distance = BALL_RADIUS * 2 + 0.1; 
                  ...
                  {stillBallMessage && (
                    <motion.div
                      ...
                      style={{
                        ...
                        top: `${((stillBallMessage.y - BALL_RADIUS - 10) / FIELD_HEIGHT) * 100}%`,
              ...
              <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="rounded-2xl w-full max-w-md shadow-2xl border backdrop-blur-xl bg-zinc-900 border-zinc-800">
                ...
                <div className="p-6 space-y-3">
                  ...
                  <div className="flex items-center justify-between p-4 rounded-xl border shadow-sm bg-zinc-950 border-zinc-800">
                    ...
                    <input type="text" value={filenamePrefix} onChange={(e) => setFilenamePrefix(e.target.value)} className="text-xs font-bold px-3 py-1.5 rounded outline-none focus:ring-1 focus:ring-emerald-500 w-32 text-right select-auto border shadow-inner bg-zinc-900 border-zinc-700 text-zinc-300" placeholder="sequence" />
                  </div>
                  <div className="flex items-center justify-between p-4 rounded-xl border shadow-sm bg-zinc-950 border-zinc-800">
